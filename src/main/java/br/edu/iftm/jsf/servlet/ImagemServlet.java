...
public class ImagemServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if(produto == null || produto.getFotoDisco() == null  || "".equals(produto.getFotoDisco())) {
            response.sendError(HttpServletResponse.SC_NOT_FOUND);
            return;
        }
        File foto = new File(produto.getFotoDisco());
        String nome  = foto.getName();
        String extencao = nome.substring(nome.lastIndexOf(".")+1, nome.length());
        String mimeType = MimeTypes.getMimeType(extencao);
        response.setContentType(mimeType);
        response.setContentLength(new Long(foto.length()).intValue());
        response.getOutputStream().write(FileUtil.fileToByteArray(foto));
